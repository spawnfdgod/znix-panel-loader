...
using znix_panel_v2_loader.Injection.Win32;

namespace znix_panel_v2_loader.Injection {
    internal class ManualMapInjector {
        ...
        private void InjectDependency( string dependency ) {
        ...
        private IntPtr GetRemoteModuleHandleA( string module ) {
            ...
            if ( dwStatus >= 0 ) {
                if ( pbi.Value.PebBaseAddress != IntPtr.Zero ) {
                    ...
                    if ( Imports.ReadProcessMemory( _hProcess, pbi.Value.PebBaseAddress + 12 /*peb.Ldr*/, out pebLdrAddress, out _ ) ) {
                        ...
                        do {
                            ...
                            if ( !Imports.ReadProcessMemory( _hProcess, new IntPtr( pLdrCurrentNode ), out lstEntryAddress, out _ ) ) {
                                Imports.HeapFree( hHeap, 0, pbi.Address );
                            }
                            ...
                            Imports.ReadProcessMemory( _hProcess, new IntPtr( lstEntryAddress ) + /*BaseDllName*/ 0x2C, out baseDllName, out _ );
                            ...
                            Imports.ReadProcessMemory( _hProcess, new IntPtr( lstEntryAddress ) + /*DllBase*/ 0x18, out dllBase, out _ );
                            Imports.ReadProcessMemory( _hProcess, new IntPtr( lstEntryAddress ) + /*SizeOfImage*/ 0x20, out sizeOfImage, out _ );
        ...
        private IntPtr GetDependencyProcAddressA( IntPtr moduleBase, PCHAR procName ) {
            ...
            Imports.ReadProcessMemory( _hProcess, moduleBase + hdrDos.e_lfanew, out hdrNt32, out _ );
            ...
            if ( expBase > 0 ) {
                ...
                for ( uint i = 0; i < expData.Value.NumberOfFunctions; i++ ) {
                    ...
                    //Debug.Assert( pName != null, nameof( pName ) + " != null" ); "antipaste".
        ...
        private bool ProcessDelayedImportTable( IntPtr baseAddress ) {
            ...
            if ( imageNtHeaders.Value.OptionalHeader.DelayImportDescriptor.Size > 0 ) {
                var imageDelayedImportDescriptor = ( PIMAGE_IMPORT_DESCRIPTOR ) RvaToPointer( imageNtHeaders.Value.OptionalHeader.DelayImportDescriptor.VirtualAddress, baseAddress );
        ...
        private void ProcessRelocation( uint imageBaseDelta, ushort data, PBYTE relocationBase ) {
            PSHORT raw;
            PDWORD raw2;
        ...
        private void ProcessSection( IntPtr baseAddress, IntPtr remoteAddress, ulong rawData, ulong virtualAddress, ulong rawSize, ulong virtualSize, uint protectFlag ) {
            if ( !Imports.WriteProcessMemory( _hProcess, new IntPtr( remoteAddress.ToInt64( ) + ( long ) virtualAddress ), new IntPtr( baseAddress.ToInt64( ) + ( long ) rawData ), new IntPtr( ( long ) rawSize ), out _ ) ) {
                return;
            }

            if ( !Imports.VirtualProtectEx( _hProcess, new IntPtr( remoteAddress.ToInt64( ) + ( long ) virtualAddress ), new UIntPtr( virtualSize ), protectFlag, out _ ) ) {
            }
        }
        ...
        private bool ProcessSections( IntPtr baseAddress, IntPtr remoteAddress ) {
            ...
            for ( ushort i = 0; i < imageNtHeaders.Value.FileHeader.NumberOfSections; i++ ) {
                ...
                if ( characteristics.HasFlag( DataSectionFlags.MemoryRead ) || characteristics.HasFlag( DataSectionFlags.MemoryWrite ) || characteristics.HasFlag( DataSectionFlags.MemoryExecute ) ) {
                    ...
                    ProcessSection( baseAddress, remoteAddress, imageSectionHeader[i].PointerToRawData, imageSectionHeader[i].VirtualAddress, imageSectionHeader[i].SizeOfRawData, imageSectionHeader[i].VirtualSize, protection );
                }
            }
        ...
        private IntPtr LoadImageToMemory( IntPtr baseAddress ) {
            ...
            if ( !ProcessDelayedImportTable( baseAddress ) ) {
#if DEBUG
